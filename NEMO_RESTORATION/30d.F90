    ...
    SELECT CASE ( kn_fwb )
      ...
      CASE (2)
        ...
        IF( MOD( kt, ikty ) == 0 ) THEN
          ...
          ! Compute mean SSH & apply drift
          a_fwb = glob_sum(e1e2t(:,:) * sshn(:,:)) / area
          drift = a_fwb - a_fwb_b
          sshn(:,:) = sshn(:,:) - drift
          WRITE(numout,*) 'sbc_fwb: drift=', drift, ' new a_fwb=', a_fwb

          ! Write new baseline for next cycle
          next_kt = kt + ikty
          WRITE(filename,'("ssh_",I7.7,".dat")') next_kt
          ioun = inum + 1
          ...
          IF( ioerr == 0 ) THEN
            ...
            CLOSE(unit=ioun)
            WRITE(numout,*) 'sbc_fwb: wrote baseline kt=', next_kt, ' a_fwb=', a_fwb
          ELSE
